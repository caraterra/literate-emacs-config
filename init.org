...
This function will switch to the scratch buffer, kill all other open buffers and tabs, and delete all but one window. I'll use this when I want to close out everything I've been working on, while still keeping Emacs open. We'll create a keybinding for this in [[*Configuring Emacs]].

#+begin_src emacs-lisp
  (defun kill-all-buffers ()
    "Kill all buffers except the scratch buffer and close all other tabs."
    (interactive)
...
#+begin_src emacs-lisp :noweb yes
  (use-package emacs
    ...
    (global-so-long-mode)
    ;; Enable global editorconfig mode
    (editorconfig-mode t)
...
This section is dedicated to configuring programming language packages. Note that while package-specific variables may be configured using the ~:custom~ directive, the setting of global variables, e.g. ~tab-width~, should be done with the mode hook, using the ~:hook~ directive.

** Configuring the Language Server Protocol Client
...
By setting ~c-default-style~, we can determine what indentation style each language mode uses. We'll have Java and AWK use their own styles, and have everything else use a custom LLVM indentation style, which is based off K&R.

We choose to use LLVM-style indentation, as this is what is used by ~clang-format~ and the ~clangd~ language server.

#+begin_src emacs-lisp
  (use-package cc-mode
    :config (add-to-list 'c-style-alist '("llvm" "k&r" (c-basic-offset . 2)))
...
By default, ~shell-mode~ uses four spaces of indentation, converting every eight spaces of indentation to a tab character. This results in a mix of tabs and spaces in files where are two or more levels of indentation. Here we force indentation using tabs with a width of 8 spaces.
